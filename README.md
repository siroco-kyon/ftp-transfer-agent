...
- `Enabled: false` でも、`Transfer.VerifyUploadedFileExists: true`（既定）なら `put` の後に宛先を確認します。**確認内容はプロトコルで異なります**: FTP は SIZE でサイズ一致を確認（SIZE 非対応サーバでは存在確認）、**SFTP は存在確認のみ**でサイズは比較しません。FTP ではサーバがファイルを保存せずに拒否した場合も転送失敗として扱われます。
...
`get` はローカルの状態も履歴も参照しないため、**取得後にローカルファイルを移動しても再取得は防げません**（リモートに残っている限り毎回取得されます）。取り込み済みを再取得したくない場合は、リモート側で対象から外す必要があります。

- `Cleanup.DeleteRemoteAfterDownload: true` にして、取得したファイルをサーバから消す
- 送信側にサーバ上で処理済みディレクトリへ移動してもらう
- `Watch.AllowedExtensions`（ワイルドカード可）で未処理ファイルだけが一致する命名規約にする
