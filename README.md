...
`FtpTransferAgent` は .NET 8 で実装されたコンソールツールで、指定したフォルダに存在するファイルを FTP もしくは SFTP 経由で転送します。アップロードだけでなくダウンロードにも対応しており、転送後はハッシュ値を検証し、必要に応じてローカルファイルの削除も行います。アプリケーションの各種挙動は `appsettings.json` によって設定できます。
