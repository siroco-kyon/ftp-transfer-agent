...
public class EndFileFeatureTests
{
    ...
    public async Task ExecuteAsync_ProcessesFilesInStableOrder()
    {
        ...
        // モックの固定ハッシュは必ず不一致となり、ハッシュ不一致はリトライ可能になったため
        // リトライ無しにして Upload 呼び出し回数の検証を安定させる
    ...
    public async Task ExecuteAsync_Get_ProcessesFileWithEndFile_WhenRequireEndFileEnabled()
    {
        ...
        mock.Setup(c => c.DownloadAsync("/remote/test.txt", It.Is<string>(p => p.EndsWith(".verify", StringComparison.Ordinal)), It.IsAny<CancellationToken>()))
    ...
    public async Task ExecuteAsync_Get_TransfersEndFiles_WhenTransferEndFilesEnabled()
    {
        ...
        mock.Setup(c => c.DownloadAsync("/remote/test.txt", It.Is<string>(p => p.EndsWith(".verify", StringComparison.Ordinal)), It.IsAny<CancellationToken>()))
            .Callback<string, string, CancellationToken>((_, lp, _) => File.WriteAllText(lp, testData))
            .Returns(Task.CompletedTask).Verifiable();
        mock.Setup(c => c.DownloadAsync("/remote/test.txt.END", It.Is<string>(p => p.EndsWith(".verify", StringComparison.Ordinal)), It.IsAny<CancellationToken>()))
