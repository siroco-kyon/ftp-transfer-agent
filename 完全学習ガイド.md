...
{
    [Required]
    [RegularExpression("^(ftp|sftp|local)$")]   // local = ローカル/UNC 共有への put
    public string Mode { get; set; } = "ftp";

    // Host/Username は local モードでは不要なので [Required] を付けず、
    // ftp/sftp での必須チェックは ConfigurationValidator 側で行う
...
`[Required]`（必須）、`[Range(1,16)]`（範囲）、`[RegularExpression("^(ftp|sftp|local)$")]`（形式）など、**1 つの項目だけで完結する**チェック。`ValidateDataAnnotations()` が担います。
...
## 5.5 3つ目の実装：LocalFileTransferClient（ローカル / UNC 宛先）

Strategy パターンの真価は「新しいやり方を、呼び出し側を変えずに足せる」ことです。実例として、`IFileTransferClient` の**3 つ目の実装** `LocalFileTransferClient`（[Services/LocalFileTransferClient.cs](FtpTransferAgent/Services/LocalFileTransferClient.cs)）があります。これは通信プロトコルではなく **OS のファイル I/O** で、ローカルフォルダや **UNC 共有（`\\server\share`）** へ書き込みます（`Mode=local`、put 専用）。

💡 **なぜ要るのか**：旧ツールは CIFS で「ローカル → LAN 共有フォルダ」へ書き込んでいました。CIFS（= SMB1）は危険なので再実装しません。が、`\\server\share` は OS が SMB を透過処理する**ただのパス**なので、専用プロトコルは不要 ── ファイル I/O で書けばよい、という発想です。

```csharp
// LocalFileTransferClient.UploadAsync（実物・要点）
var temp = $"{destination}.tmp.{Guid.NewGuid():N}";   // SFTP/FTP と同じ「一時名」規約
await CopyFileAsync(localPath, temp, ct);              // ストリームコピー（大容量でもメモリ一定）
File.Move(temp, destination, overwrite: true);        // 完了後に原子的リネーム
```

💡 ここがまさに第5回の総まとめです。**「一時名 → リネーム → 失敗時に掃除」「ハッシュ検証」という"考え方"は全実装で共通**で、違うのは「実際に運ぶ手段（SSH.NET / FluentFTP / `File` API）」だけ。`Worker` は相変わらず `IFileTransferClient` としか会話しないので、local 実装を足しても**司令塔のコードは無変更**です。`CreateClientCore` の `switch` に `"local"` を 1 行足し、`BuildRemotePath` に「OS ネイティブのパスで組む」分岐を足しただけで統合できました。

⚠️ local は **put 専用**（get 経路はリモートパスを `/` 前提で解決するため）。詳細・設計判断は [docs/local-destination.md](docs/local-destination.md) を参照。

🧪 **演習（第5回）**：`SftpClientWrapper.UploadAsync`・`AsyncFtpClientWrapper.UploadAsync`・`LocalFileTransferClient.UploadAsync` の 3 つを並べて読み、「共通する考え方（一時名→リネーム→失敗時に掃除）」と「手段（ライブラリ/API）の違い」を仕分けしてみましょう。Strategy パターンが「共通の考え方を 1 つのインターフェースに、違いを各実装に閉じ込める」ものだと実感できます。
