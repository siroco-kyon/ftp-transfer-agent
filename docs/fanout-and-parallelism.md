...
## 5.6 具体例を“実物”で最後まで追う（初学者向け・上長説明用）

§5 は流れの要約だった。ここでは **実際に作られるファイル名・フォルダ・JSON の中身を本物の値で** 示し、初めて読む人でも「で、結局ディスクに何が残るの？」が手に取るように分かることを目標にする。プログラムの知識は前提にしない。
...
ファイル名が長いハッシュなのは §5.6.7 で説明する。**1 ファイル名 = 「どのファイルを・どの宛先に送ったか」の組 1 つ**に対応している。
...
初学者・上長から必ず出る質問を、実物の動きで答える。

**(a) 送信後に同じ名前で中身を差し替えたら？（例: `report.csv` を新しい内容で上書き）**

→ 次回起動の指紋計算で、サイズか更新時刻が変わるので指紋が変わる（例: `st:20480-639177159000000000`）。付箋の `Signature` と**不一致**になる。プログラムは「これは別物だ」と判断し、その**古い付箋をその場で削除して、全宛先へ送り直す**。古い内容しか届いていない宛先に新しい内容を取りこぼさないための仕掛け。
