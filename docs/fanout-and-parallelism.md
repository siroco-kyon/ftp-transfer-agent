...
## 5.6 具体例を“実物”で最後まで追う（初学者向け・上長への説明用）

§5 では流れの全体像を示した。ここでは **実際に作られるファイル名・フォルダ・JSON の中身を本物の値で** 示し、初めて読む人でも「で、結局ディスクに何が残るのか」が手に取るように分かることを目標にする。プログラムの知識は前提にしない。

### 5.6.0 先に用語を 3 つだけ（これだけ分かれば読める）

| 用語 | かみくだくと | この資料での例え |
|---|---|---|
| **指紋（Signature）** | 「送った時点のそのファイル」を表す短い文字列。既定の `sizetime` では**サイズと最終更新時刻**から作る。 | ファイルの“今この瞬間の姿”を写した**証明写真** |
| **マーカー（marker）** | 「このファイルは、この宛先には、もう送り終えた」という事実を 1 件記録した**小さな JSON ファイル**。 | 配達済みを示す**付箋（ふせん）** |
| **状態ディレクトリ** | マーカーを置いておく専用フォルダ。`Watch.Path`（監視フォルダ）とは**別の場所**。 | 付箋を貼っておく**掲示板** |

> ⚠️ **指紋モードの注意**: 既定の `sizetime` は **サイズと最終更新時刻だけ**を見る。だから **サイズも更新時刻も変えずに中身だけ差し替えた**場合（タイムスタンプを保持してコピーする運用など）は指紋が変わらず、「配信済み」のままスキップされて再送されない。**どんな内容変更も確実に検知したい場合は `DeliverySignatureMode: "hash"`**（ファイルのハッシュを指紋にする。指紋の接頭辞が `st:` → `h:` になる）を使う。`hash` は毎回全バイトを読むため I/O は増えるが、検知は厳密になる。

ひとことで言うと、このツールは **「まだ配り切れていない荷物にだけ付箋を貼る」**。全部配れたら付箋は剝がす。だから正常に全宛先へ送れた日常運用では、状態ディレクトリは**空っぽ**になる。

### 5.6.1 この例の前提（設定とファイル）

- 監視フォルダ（`Watch.Path`）: `C:\data\outbox`
- 同じ 1 ファイル `report.csv`（サイズ 12,345 バイト、最終更新 2026-06-18 01:23:45 UTC）を **3 宛先**へ配る。
- 既定の `DeleteAfterVerify: true`（送れたらローカルの元ファイルは消す運用）。

`appsettings.json`（関係する部分だけ抜粋）:
...
```jsonc
{
  "Transfer": {
    ...
    // ↓ 追加宛先を 1 つ以上書くと「ファンアウト（複数配信）」になり、
    //    自動的に「宛先別配信トラッキング（＝マーカーの仕組み）」が有効になる
    "AdditionalDestinations": [
      { "Name": "osaka", "Mode": "ftp", "Host": "osaka.example.co.jp", "Port": 21, "RemotePath": "/incoming", "Concurrency": 2 },
      { "Name": "tokyo", "Mode": "ftp", "Host": "tokyo.example.co.jp", "Port": 21, "RemotePath": "/incoming", "Concurrency": 2 }
    ]

    // "StateDirectory" は未指定 → 既定の場所（後述）に自動で作られる
    // "DeliverySignatureMode" は未指定 → 既定の "sizetime"
  }
}
```

> ポイント: **追加宛先（`AdditionalDestinations`）を 1 つでも書いた時点で、マーカーの仕組みが自動で動き出す。** 追加宛先がなければ単一転送先で、マーカーは作られない（失敗したファイルは `Watch.Path` にそのまま残るだけ。§5.5 の表を参照）。

### 5.6.2 マーカーが置かれる場所（実物のパス）

`StateDirectory` を指定していないので、既定の場所が使われる。Windows でユーザー `svc-ftp` が動かしている場合:

```
C:\Users\svc-ftp\AppData\Local\FtpTransferAgent\delivery-state\3b6d537fff47b5c1\
                                                               └─ ① watch パス由来のハッシュ 16 桁
```

① の `3b6d537fff47b5c1` は **監視フォルダのフルパス `c:\data\outbox` を SHA256 にかけた先頭 16 桁**。監視フォルダごとにサブフォルダを分け、別の監視フォルダのマーカーと混ざらないようにするための工夫（自分で名前を考える必要はなく、自動でこうなる）。

### 5.6.3 Run 1 — osaka がメンテで落ちている

`report.csv` を置いて実行する。primary と tokyo へは送れたが、osaka は接続失敗（リトライも尽きた）。

**この瞬間に状態ディレクトリへ作られる「実物」はこの 2 ファイル:**

```
C:\Users\svc-ftp\AppData\Local\FtpTransferAgent\delivery-state\3b6d537fff47b5c1\
├─ 81aa9f60c048913876fae8e5556a9899fcc15128040fb0d13511a3ecae0ec364.marker   ← report.csv × primary
└─ 3060eb0d38e2166747461c1e926b129a9cefe4875fefd09242167241f82c2a1b.marker   ← report.csv × tokyo
   （osaka のマーカーは作られない。送れていないから）
```

ファイル名が長いハッシュなのは §5.6.7 で説明する。**1 つのファイル名が「どのファイルを、どの宛先へ送ったか」という組 1 つ**に対応している。

`81aa9f60...marker` の**中身（実際の JSON）**:

```json
{"RelativePath":"report.csv","DestinationName":"primary","Signature":"st:12345-639173426250000000","DeliveredAtUtc":"2026-06-18T01:23:45Z"}
```
...
| キー | 値 | 意味 |
|---|---|---|
| `RelativePath` | `report.csv` | 監視フォルダから見た相対パス（どのファイルか） |
| `DestinationName` | `primary` | どの宛先へ送れたか |
| `Signature` | `st:12345-639173426250000000` | 送った時点の指紋。`st:` = sizetime、`12345` = バイト数、`-` の後ろ = 最終更新時刻（.NET の Ticks 値） |
| `DeliveredAtUtc` | `2026-06-18T01:23:45Z` | 送れた日時（UTC） |

`tokyo` のマーカー（`3060eb0d...marker`）は `DestinationName` が `"tokyo"` になるだけで、あとは同じ。

**元ファイル `report.csv` はどうなる？** osaka へ未達なので、`C:\data\outbox\report.csv` は **リトライフォルダへ移動**される（既定では `...\FtpTransferAgent\delivery-retry\3b6d537fff47b5c1\report.csv`）。`Watch.Path` から退避させることで、次回の通常列挙が二重に拾わないようにしている（詳細は §5.5）。**終了コードは 1**（部分失敗）。
...
Run 1 の結果（ディスクに残るもの）
─────────────────────────────────
状態ディレクトリ:  primary の付箋 ●   tokyo の付箋 ●   （osaka は無し）
元ファイル:        C:\data\outbox\report.csv は retry フォルダへ移動して保管
意味:              「report.csv は primary と tokyo へは配達済み。osaka がまだ」
```

### 5.6.4 Run 2 — osaka が復旧（タスクスケジューラで翌回起動）

osaka が復旧した状態でもう一度起動する。何が起きるかを順に追う。

1. **起動時**: 状態ディレクトリを 1 回走査して、さっきの 2 枚の付箋をメモリに読む。元ファイルは retry フォルダにあるので「孤児（元が消えた付箋）」ではない＝有効。
2. **指紋の再計算**: いま手元の `report.csv` の指紋を計算する。中身を変えていなければ `st:12345-639173426250000000` のまま。
3. **照合**: 計算した指紋と、primary・tokyo の付箋の `Signature` を**突き合わせる → 一致**。よって「配信済み = {primary, tokyo}」。
4. **差分だけ送る**: `pending = 全宛先 − 配信済み = {osaka}`。**osaka にだけ**送る（primary/tokyo へは送らない＝二重配信しない）。
5. **完了**: osaka 成功 → これで {primary, tokyo, osaka} = 全宛先そろった＝**完了**。
   - `DeleteAfterVerify: true` なので元ファイルを削除。
   - **付箋（primary・tokyo のマーカー）も全部剝がす**。
   - **終了コードは 0**。

**Run 2 のあと、状態ディレクトリはこうなる:**
...
これがいちばん大事な性質: **全宛先に配り切ると、マーカーは残らない。** つまり「状態ディレクトリにマーカーがある＝まだ配り切れていない案件がある」という見方ができる（運用監視のヒントにもなる）。

### 5.6.5 よくある“もしも”を実物で

初学者・上長から必ず出る質問に、実際の動きに即して答える。

**(a) 送信後に同じ名前で中身を差し替えたら？（例: `report.csv` を新しい内容で上書き）**

→ 次回起動時の指紋計算で、サイズか更新時刻が変わっているため指紋も変わる（例: `st:20480-639177159000000000`）。付箋の `Signature` と**一致しなくなる**。プログラムは「これは別物だ」と判断し、その**古い付箋をその場で削除して、全宛先へ送り直す**。送信後に差し替えられた新しい内容を、どの宛先にも取りこぼさず届け直すための仕掛けである。

> ただし既定の `sizetime` は **サイズと更新時刻でしか比べない**。**サイズも更新時刻も同じまま中身だけ差し替えた**ケース（例: タイムスタンプを保持するコピーツールでの上書き）は指紋が変わらず、差し替えを検知できずスキップされる。この取りこぼしを防ぎたいなら `DeliverySignatureMode: "hash"` を使う（§5.6.0 の注意を参照）。

**(b) 部分失敗のあと、元ファイルを人手で消したら？**

→ 付箋だけが残り、対応する元ファイルが無い状態（＝**孤児マーカー**）。次回起動時の走査で**自動的に掃除**される（その案件はもう追わない）。

**(c) `DeleteAfterVerify: false`（送ってもローカルに残す保持運用）の場合は？**

→ ここだけ例外。**全宛先に成功してもマーカーを残す**。元ファイルが残り続けるので、次回また全宛先へ送らないよう「もう送ったよ」という付箋（全宛先分）を貼っておく必要があるため。この運用では「全部成功 → 状態ディレクトリに全宛先分のマーカーが残る」が正常。

### 5.6.6 Run 1 → Run 2 を 1 枚の図で

```mermaid
flowchart TB
    subgraph R1["Run 1（osaka ダウン）"]
        direction TB
        a1["report.csv を 3 宛先へ"] --> a2["primary ✅ / tokyo ✅ / osaka ❌"]
        a2 --> a3["状態Dir に付箋 2 枚<br/>81aa9f60….marker（primary）<br/>3060eb0d….marker（tokyo）"]
        a2 --> a4["元ファイルは retry フォルダへ退避"]
        a2 --> a5["終了コード 1"]
    end
    R1 -->|翌回起動| R2
    subgraph R2["Run 2（osaka 復旧）"]
        direction TB
        b1["指紋を再計算 → 2 枚の付箋と一致<br/>配信済み = {primary, tokyo}"] --> b2["pending = {osaka} だけ送る"]
        b2 --> b3["全宛先そろった＝完了"]
        b3 --> b4["元ファイル削除 ＋ 付箋を全部剝がす"]
        b4 --> b5["状態Dir は空・終了コード 0"]
    end
```

### 5.6.7 なぜファイル名が長いハッシュなのか（1 分で）

マーカーのファイル名は **`SHA256("report.csv" + 区切り + "primary")` を 16 進小文字にしたもの** + `.marker`。なぜそのまま `report.csv__primary.marker` のような名前にしないのか:

- 相対パスには `\` や `/`、空白、日本語などが入り得る。そのままファイル名にすると OS の制約や文字化けで壊れる。
- サブフォルダ付きの相対パス（例 `invoice\2026-06.csv`）も、ハッシュにすれば**必ず安全な 1 個の平らなファイル名**になる。
- 同じ (ファイル, 宛先) からは**毎回まったく同じ名前**が決定論的に出るので、次回も同じマーカーを正しく引ける。

つまりハッシュ名は「人が読むため」ではなく「どんなパス名でも安全・一意・再現可能なファイル名にするため」。中身（JSON）の `RelativePath` / `DestinationName` を見れば、人にもどの案件か分かる。

### 5.6.8 上長へのひとこと要約

> 同じファイルを複数拠点へ配るとき、**配り切れなかった分だけ「配達済みメモ（マーカー）」を専用フォルダに残します。** 次回起動時はそのメモと突き合わせ、**まだ届いていない拠点にだけ**送り直します。全拠点へ届いたらメモは自動で消え、元ファイルも片付きます。さらに、送ったあとにファイルの中身が差し替わった場合はメモが無効化され、全拠点へ確実に送り直します。**「メモが残っている＝未完了の配信がある」** とご理解いただければ十分です。
