...
FtpTransferAgent は、指定したローカルフォルダ内のファイルを FTP または SFTP サーバーへ一括転送（または逆方向でダウンロード）する .NET 10 ベースのコンソールアプリケーションです。**バッチ処理型**として設計されており、起動時に処理対象ファイルを検出し、設定に従って転送・検証・後処理を実行してから自動的に終了します。
...
- 各宛先へ同一内容を配信するため、列挙時のデータファイルと関連 END ファイルを一時スナップショット化し、全宛先はそのスナップショットを読み取る。関連 END ファイルがある場合は END 側の指紋も配信トラッキングの判定に含める
- **全宛先成功時に**ローカルファイルを削除（`Cleanup.DeleteAfterVerify: true`、既定）。`false` では元ファイルを保持
- スナップショット作成中、または全宛先完了後の cleanup/retry 判定前に元ファイルや関連 END ファイルの指紋が変わった場合は、ローカル削除・retry 退避を行わず、終了コード `1` で次回実行に持ち越す
...
- 複数宛先では、ファイル列挙後に一時スナップショットを作成し、全宛先が同じスナップショットを送信します。これにより並列アップロード中に元ファイルや関連 END ファイルが上書きされても、宛先間で内容が割れないようにします。
- 関連 END ファイルがある場合、マーカーにはデータファイル指紋と END ファイル指紋を合成した値を記録します。END ファイルだけが差し替わった場合も古いマーカーを配信済み扱いにしません。
- スナップショット作成前後、完了処理時、または全宛先配信済みとして送信をスキップした後の cleanup 前に元ファイルや関連 END ファイルの指紋が変わった場合、その実行ではローカル削除・retry 退避を行いません。次回実行で新しい指紋として再評価します。
...
- **転送中の元ファイル/関連 END ファイル変更は次回へ持ち越し**: 複数宛先ではスナップショットにより宛先間の内容差異は防ぐが、元ファイルや関連 END ファイルが変更された実行では cleanup/retry を確定せず終了コード `1` とする
