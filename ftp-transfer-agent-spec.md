...
- 🔒 **ハッシュ値による整合性検証**（SHA256/SHA512、MD5は非推奨）
...
| 項目 | 型 | 必須 | 説明 | デフォルト値 | 制約 |
|------|-----|------|------|--------------|------|
| Algorithm | string | ✓ | ハッシュアルゴリズム | "SHA256" | "SHA256" または "SHA512" |
...
**バージョン**: 2.2.0  
**主な更新内容**:
- ENDファイル機能をGet操作にも対応（Put/Get両対応）
- セキュリティ強化：MD5ハッシュアルゴリズムを非推奨化、SHA256/SHA512を推奨
- パストラバーサル攻撃対策の強化（ダウンロード処理）
- 設定検証機能の拡充（ConfigurationValidator.cs）
- テスト実行性能の大幅改善（2分から1分以下に短縮）
- 142テスト全体の安定性向上と包括的なテストカバレッジ
- リモートファイルパスのUnix形式対応強化
- 例外処理とエラーハンドリングの改善
