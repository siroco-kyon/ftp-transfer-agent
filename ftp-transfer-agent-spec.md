...
- 🔒 **ハッシュ値による整合性検証**（SHA256/SHA512対応）
- 🔄 **Polly による自動再試行機能**（指数バックオフ）
- 🗑️ **転送成功後の自動削除オプション**
- ⚡ **並列転送対応**（最大16ファイルの同時転送、Channel + TransferQueue 実装）
- 🎯 **ENDファイル制御機能**（ENDファイルがある場合のみ転送対象にする制御、Put/Get両対応、順序保証付き）
...
| TransferEndFiles | boolean | - | ENDファイル自体も転送するか（自動削除付き） | false |
...
- 例：`test.txt` を転送するには `test.END` または `test.end` が同じディレクトリ（ローカル）またはリモートパスに存在する必要があります
- ENDファイルの拡張子は `EndFileExtensions` で設定可能（デフォルト: `.END`, `.end`）
- この機能は**アップロード処理（Direction: "put"）とダウンロード処理（Direction: "get"）の両方で有効**です
...
| Direction | string | ✓ | 転送方向 | "put" | "get" / "put" |
...
| 項目 | 型 | 必須 | 説明 | デフォルト値 | 制約 |
|------|-----|------|------|--------------|------|
| Algorithm | string | ✓ | ハッシュアルゴリズム | "SHA256" | "SHA256", "SHA512" |
...
| DeleteRemoteEndFiles | boolean | - | 転送先ENDファイルを転送成功後に削除するか | false |
...
2. **ファイル列挙**（Direction に応じて実行）
   ...
   - **"get" モード**: RemotePath 内のファイルリストを `client.ListFilesAsync` で取得
     - ENDファイル機能が有効な場合、対応するENDファイルが存在するファイルのみが対象
     - ENDファイル検証はリモートファイル一覧内での検索で実施
...
6. **後処理**
   - 検証成功時：
     - アップロード時：設定に応じてローカルファイルを削除（転送元ENDファイルは常に削除）、転送先ENDファイルはDeleteRemoteEndFiles設定に従って削除
     - ダウンロード時：設定に応じてリモートファイルを削除、ENDファイルはDeleteRemoteEndFiles設定に従って削除
   - 検証失敗時：エラーログを出力（メール通知も送信）

7. **終了**
   - パフォーマンス監視タスクをキャンセルして適切に終了
   - すべての転送処理が完了したら `_lifetime.StopApplication()` を呼び出し
   - 1件でも転送失敗が記録された場合はプロセス終了コードを `1` に設定
   - アプリケーションが自動的に終了
...
**バージョン**: 2.4.0  
**主な更新内容**:
- ENDファイル転送先削除機能：DeleteRemoteEndFilesオプションを追加し、転送先ENDファイルの削除を設定で制御可能
- ENDファイル削除ロジックの改善：転送元ENDファイルは常に削除、転送先ENDファイルは設定で制御
- アプリケーション終了問題の修正：パフォーマンス監視タスクが適切に終了するよう修正
- MD5ハッシュは設定値として無効化し、SHA256/SHA512のみをサポート
- ENDファイル処理の改善：拡張子フィルタリングがENDファイルに影響しないよう修正
- リモートファイルパス処理の修正：Getモードでのパス構築問題を解決
- 全テスト140個が正常にパス（成功率100%）
- 設定ファイルにTransferEndFilesオプションとDeleteRemoteEndFilesオプションを追加
- バッチ処理型の動作を確実に保証（定期実行対応）
