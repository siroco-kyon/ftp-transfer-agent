...
| AdditionalDestinations | object[] | - | `[]` | - | put 方向の追加送信先（[5.3](#53-複数宛先への同時配信ファンアウト) 参照）。各要素は Transfer と同じ接続系プロパティを持つ（`Direction` / `AdditionalDestinations` を除く）。`Concurrency` / `TimeoutSeconds` / 認証はその宛先に個別適用 |
...
- 部分失敗時の再送は **all-or-nothing**。失敗宛先だけでなく成功済み宛先にも再送する
...
- **転送の中断/再開・履歴の永続化は未対応**（部分失敗は次回実行で再送）
