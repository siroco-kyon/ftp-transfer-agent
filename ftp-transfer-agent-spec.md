...
FtpTransferAgent は、指定したローカルフォルダに存在するファイルを FTP または SFTP サーバーへ転送する .NET 8 ベースのコンソールツールです。転送後はハッシュ値による整合性チェックを行い、必要に応じて転送済みファイルの自動削除も可能です。双方向転送（アップロード/ダウンロード）にも対応しています。

### 主な特徴
- ❎ リアルタイムフォルダ監視（タスクスケジューラでの定期実行を前提）
- 🚀 起動時に既存ファイルを自動転送
...
#### 4.2.1 Watch（対象フォルダ設定）

転送対象となるフォルダと条件を設定します。
...
| 項目 | 型 | 必須 | 説明 | デフォルト値 |
|------|-----|------|------|--------------|
| Path | string | ✓ | 対象フォルダのパス（相対/絶対パス） | なし |
| IncludeSubfolders | boolean | - | サブフォルダも対象とするか | false |
...
- **"get"**: リモート → ローカル（ダウンロードのみ）
 - **"both"**: 双方向転送（起動時にダウンロードし、その後ローカルファイルをアップロード）
...
1. **起動時の初期化**
   ...
   - FTP/SFTP クライアントの初期化
   - 転送キューの初期化（並列度の設定）
    - ローカルフォルダの列挙（put/both モード時）
    - 列挙したファイルのアップロード（put/both モード時）
   - 既存ファイルのダウンロード（get/both モード時）

2. **アップロード対象ファイルの判定**
    - フォルダ内のファイルを拡張子フィルターで選別
    - 追加されたファイルを転送キューへ登録
