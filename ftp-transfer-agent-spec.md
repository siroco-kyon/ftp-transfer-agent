...
- 🎯 **宛先別配信トラッキング**（複数宛先で**既定有効**。未配信の宛先だけ再送、上書き検出付き）
- 🗑️ **転送成功後の自動削除オプション**（ローカル/リモート、ENDファイル個別制御。`DeleteAfterVerify` は既定 true）
...
| 項目 | 型 | 必須 | 既定値 | 制約 | 説明 |
|------|-----|------|--------|------|------|
| Name | string | 複数宛先で✓ | null | - | 宛先の安定識別子。**複数宛先（ファンアウト）の put では primary・追加宛先のすべてで必須かつ一意**（配信トラッキングが常時有効なため）。単一宛先でも `PerDestinationDeliveryTracking: true` なら必須。接続情報と独立した名前にする（[5.9](#59-宛先別配信トラッキングput) 参照） |
...
| 項目 | 型 | 必須 | 既定値 | 説明 |
|------|-----|------|--------|------|
| LockFilePath | string | - | `""` | 二重起動防止用ロックファイルパス。空の場合は `Watch.Path` ごとに分離した既定パス `LocalApplicationData/FtpTransferAgent/locks/<watch パスのハッシュ>/ftp-transfer-agent.lock`（取得不能環境ではテンポラリ配下）を使用。異なる監視フォルダの並走が相互ブロックしない |
...
| SuppressPerDestinationFailureDetailEmails | bool | - | false | 複数宛先での**個別宛先失敗の詳細**エラーメールを抑制する。ある宛先がメンテ等で継続的に失敗しても詳細通知を止め、設定不備・認証など他のエラーメールは送信し続ける。**ファイル単位の部分配信サマリ通知は抑制対象外**（どのファイルがどの宛先へ未配信かは引き続き通知される。送信過多は `MaxEmailsPerRun` で抑制）。ファイルログ・終了コードには影響しない（[5.9](#59-宛先別配信トラッキングput) 参照） |
...
6. **後処理**
   ...
   - 失敗時: エラーログ出力（メール通知）。ファンアウト部分失敗時は配信トラッキングが成功済み宛先を記録し、対象ファイルを `RetryDirectory` へ退避。次回実行では未配信の宛先だけへ再送する
...
- **Polly 再試行**: リトライ可能な例外のみ指数バックオフで再試行（恒久的な例外＝認証・パス不正・`UnauthorizedAccessException` 等は即時失敗）
- **例外分離**: 一部のファイルが失敗しても他の処理は継続（ワーカー単位で隔離）
- **宛先間の独立投入**: ファンアウト時はキュー投入を宛先ごとの独立プロデューサで行う。ある宛先のチャネルが詰まっても他宛先への投入はブロックされない（1 宛先の不調が全体を律速しない）
...
- 各宛先は専用のキューとクライアントを持ち、`Concurrency` を個別に適用。キュー投入は宛先ごとに独立（[5.2](#52-並列転送の仕組みchannel--transferqueue)）
- 各宛先に**一意な `Name` が必須**（配信トラッキングのマーカーキー。未設定なら起動時バリデーションエラー）
- **全宛先成功時に**ローカルファイルを削除（`Cleanup.DeleteAfterVerify: true`、既定）。`false` では元ファイルを保持
- `Direction: get` では追加宛先は使用されず、警告が表示される

**部分失敗時は「未配信の宛先だけ再送」が常時の挙動です（成功済み宛先への一括再送は行いません）。**

- 複数宛先のファンアウトでは**宛先別配信トラッキングが常時有効**。`PerDestinationDeliveryTracking` フラグの値に関わらず、配信済みの宛先を記録し、次回バッチでは**未配信の宛先にだけ**再送する。
- 一部宛先だけ失敗したファイルは `RetryDirectory` へ退避し、未配信宛先へ再送し続ける。全宛先完了後、`DeleteAfterVerify: true`（既定）なら削除、`false` なら `Watch.Path` の元位置へ復元する。
- 詳細は [5.9](#59-宛先別配信トラッキングput) を参照。
...
複数宛先（[5.3](#53-複数宛先への同時配信ファンアウト)）で、ある宛先がメンテナンス等で一時的にダウンしても、**復旧済みの宛先には再送せず、未配信の宛先だけに送り直す**ための仕組みです。**複数宛先（ファンアウト）の put では常時有効**で、無効化はできません（成功済み宛先への一括再送という事故を防ぐため）。単一宛先で利用したい場合のみ `Transfer.PerDestinationDeliveryTracking: true` を指定します（put 方向のみ）。
...
- retry 移動時は `sizetime` 署名が変わらないよう、退避先で元の最終更新時刻を保持します（クロスボリューム移動でも全再送になりません）。
- 全宛先のマーカーが揃った時点で、`Cleanup.DeleteAfterVerify: true`（既定）ならローカルファイル・マーカーを削除します。`false` の場合は、retry へ退避していたファイルを `Watch.Path` の元の位置へ復元し（隠しフォルダに取り残しません）、マーカーは保持して次回の再送をスキップします。
...
**宛先の識別（Name 必須）**:
- マーカーは宛先の `Name` をキーにします。そのため **primary を含む全宛先で `Name` が必須かつ一意**です（未設定・重複は起動時にエラー）。複数宛先では常時トラッキングのため、必ず `Name` を付けてください。
- 接続情報（ホスト・ポート・パス）ではなく独立した `Name` を使うことで、サーバ移転やパス変更後も「同じ宛先」と認識し続けます。

**通知・終了コード**:
- 宛先失敗は既定どおりエラーとして記録され、エラーメール送信と終了コード `1` の対象になります。
- ある宛先が継続的に失敗して個別の詳細メールが煩わしい場合は `Smtp.SuppressPerDestinationFailureDetailEmails: true` で、**個別宛先失敗の詳細メール**を抑制できます。**ファイル単位の部分配信サマリ通知は抑制対象外**なので、どのファイルがどの宛先へ未配信かは引き続き把握できます。設定不備・認証エラーなど他のエラーメールも送信され続けます。**ファイルログと終了コードには影響しません**（障害は引き続き記録・検知できます）。

**注意・既知の限界**:
- `Cleanup.DeleteAfterVerify: false` の場合、全宛先へ配信済みでもローカルを残します（マーカーも保持され、その分は再送をスキップ）。部分失敗で retry へ退避していたファイルは、全宛先完了時に `Watch.Path` の元位置へ復元されます。
...
**バージョン**: 3.2.0
**主な更新内容 (3.2.0)**:
- **複数宛先（ファンアウト）の put で宛先別配信トラッキングを既定有効化**。成功済み宛先への一括再送（all-or-nothing）を廃止し、未配信の宛先だけ再送する挙動を標準とした。複数宛先では各宛先の `Name` が必須・一意（起動時バリデーション）
- **`Cleanup.DeleteAfterVerify` の既定を `true` に変更**（アウトボックス運用）。`false` 指定時、部分失敗で retry へ退避したファイルは全宛先完了時に `Watch.Path` の元位置へ復元（隠しフォルダに取り残さない）
- **キュー投入を宛先ごとに独立化**。ある宛先のチャネルが詰まっても他宛先への投入が止まらない
- **`Smtp.SuppressPerDestinationFailureDetailEmails` の抑制範囲を是正**。個別宛先失敗の詳細のみ抑制し、ファイル単位の部分配信サマリ通知は残す
- **retry 退避時に `sizetime` 署名の最終更新時刻を保持**（クロスボリューム移動でも全再送にならない）
- **例外分類の改善**: `UnauthorizedAccessException` を非リトライ化、FTP は応答コード（4xx/5xx）を優先判定
- **二重起動ロックの既定パスを `Watch.Path` ごとに分離**（異なる監視フォルダの並走が相互ブロックしない）
