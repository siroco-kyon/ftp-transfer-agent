...
- ハッシュ検証による転送後確認
- END ファイルを使った受け渡し制御
- **複数宛先への同時配信(ファンアウト)+ 配信トラッキング**
- **並列転送(最大 16・接続再利用)**、再試行、転送後削除、ログ出力、メール通知
...
1 つのファイルを **メイン + 追加宛先のすべてへ同時送信**(put のみ)。

- 宛先ごとに **プロトコル・認証・並列数を個別設定**(SFTP / FTP 混在可)
- **全宛先成功時のみ**ローカル削除。1 つでも失敗したら残す
- **配信トラッキング**: どの宛先まで送れたかをファイル単位で記録し、
  次回は**未配信の宛先にだけ**再送(配信済みへは二重送信しない)
- 送信後に元ファイルが差し替わったら指紋で検知し、全宛先へ送り直す

> 用途例: 本番 + バックアップ、複数拠点への一斉配信。
> ある拠点が落ちても、復旧後その拠点だけ送り直せる。

---

## 並列処理

最大 **16 並列**で同時転送しつつ、**並列でも壊れない**設計。

- **ワーカー隔離** — 1 件の失敗が他の転送を止めない(失敗分だけエラー)
- **接続の再利用** — 1 件ごとに繋ぎ直さず接続を使い回し、確立コストを削減
- **宛先ごとに並列数を指定** — 速い宛先は多め、弱い宛先は控えめに
- **宛先間が独立** — ある宛先が詰まっても健全な宛先は止まらない

> 「速く」かつ「1 つの失敗・遅延が全体に波及しない」を両立。
